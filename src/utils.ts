...
const hasEmojiShortCode = (emoji: Emoji): emoji is Emoji & { shortCode: string; value: Plain } =>
	!!emoji.value && 'shortCode' in emoji;

const joinEmoji = (current: Inlines, previous: Inlines | undefined, next: Inlines | undefined): Inlines => {
	if (current.type !== 'EMOJI' || !hasEmojiShortCode(current) || (!previous && !next)) {
		return current;
	}
	...
	if (hasEmojiAsNeighbor || hasPlainAsNeighbor) {
		if (isEmoticon) {
...
export const extractFirstResult = (value: Types[keyof Types]['value']): Types[keyof Types]['value'] => {
	...
	const result = value.find(Boolean) as Types[keyof Types]['value'];
	if (value.length > 1) {
		console.log('extractFirstResult called with array of length', value.length, 'returning first element only');
	}
	return result;
};
