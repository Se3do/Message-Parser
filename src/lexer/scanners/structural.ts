...
import { CH_LBRACKET, CH_RBRACKET, CH_LPAREN, CH_RPAREN, CH_BACKSLASH, CH_UNDERSCORE } from '../constants/charCodes';
...
export function scanEscape(ctx: ScanContext, pos: number): number {
    ...
    // `\\` + `_` (shruggie etc.): one displayed `\` then a literal underscore (not italic)
    if (next === CH_BACKSLASH && ctx.input.charCodeAt(pos + 2) === CH_UNDERSCORE) {
        flushText(ctx, pos);
        emit(ctx, TokenKind.ESCAPED, '\\\\', '\\', pos);
        emit(ctx, TokenKind.ESCAPED, '\\_', '_', pos + 1);
        return pos + 3;
    }

    // escapable char — peg strips backslash for *_~`#.\\; keeps `\` + char for brackets etc.
    if (next < 128 && ESCAPABLE[next]) {
        flushText(ctx, pos);
        const escaped = ctx.input[pos + 1];
        const raw = `\\${escaped}`;
        const dropBackslash = '*_~`#.\\'.includes(escaped);
        emit(ctx, TokenKind.ESCAPED, raw, dropBackslash ? escaped : raw, pos);
        return pos + 2;
    }
